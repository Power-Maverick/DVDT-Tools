...
export class ERDGenerator {
    ...
    private generateMermaid(solution: DataverseSolution): string {
        ...
        for (const table of solution.tables) {
            ...
            if (this.config.includeAttributes && table.attributes.length > 0) {
                ...
                const attributes = this.config.maxAttributesPerTable > 0 ? table.attributes.slice(0, this.config.maxAttributesPerTable) : table.attributes;

                for (const attr of attributes) {
                    const type = this.mapToMermaidType(attr.type);
                    const attrName = this.sanitizeTableName(attr.logicalName);
                    const keyToken = attr.isPrimaryId && !table.isIntersect ? " PK" : "";
                    const commentToken = attr.isRequired && !attr.isPrimaryId ? ' "required"' : "";
                    lines.push(`        ${type} ${attrName}${keyToken}${commentToken}`);
                }
                ...
                // Mermaid ER syntax does not allow empty blocks. Emit a single placeholder attribute.
                lines.push(`    ${tableName} {`);
                lines.push(`        string no_visible_attributes`);
                lines.push(`    }`);
            }
        }
    ...
    private generateGraphviz(solution: DataverseSolution): string {
        ...
        // Add tables
        for (const table of solution.tables) {
            const attributes = this.config.includeAttributes ? (this.config.maxAttributesPerTable > 0 ? table.attributes.slice(0, this.config.maxAttributesPerTable) : table.attributes) : [];
    ...
    private generateDrawio(solution: DataverseSolution): string {
        ...
        for (const table of solution.tables) {
            ...
            const attributeCount = this.config.includeAttributes
                ? Math.min(table.attributes.length, this.config.maxAttributesPerTable > 0 ? this.config.maxAttributesPerTable : table.attributes.length)
                : 0;
            ...
            // Create entity cell
            lines.push(
                `        <mxCell id="${entityId}" value="${this.escapeXml(htmlContent)}" style="shape=swimlane;fontStyle=0;childLayout=stackLayout;horizontal=1;startSize=30;horizontalStack=0;resizeParent=1;resizeParentMax=0;resizeLast=0;collapsible=1;marginBottom=0;whiteSpace=wrap;html=1;fillColor=#dae8fc;strokeColor=#0e639c;" vertex="1" parent="1">`,
            );
